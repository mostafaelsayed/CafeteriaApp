        ...
        foreach ($menuItems as $key => $menuItem) {
            ...
            foreach ($flattenOrderItems as $key => $OrderItem) {
                if ($menuItem['Id'] === $OrderItem['MenuItemId']) {
                    // remove from front end also as at the end , you'll update the visible only in front and back
                    if (!$menuItem['Visible']) {
                        foreach ($orders as $key => $order) {
                            // send notification foreach user by items removal
                        ...
                        foreach ($orders as $key => $order) {
//send notification foreach user by price changes
                            if ($order['Id'] === $OrderItem['OrderId']) {
